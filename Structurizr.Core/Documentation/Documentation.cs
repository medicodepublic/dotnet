...
namespace Structurizr.Documentation
{
    ...
    public sealed class Documentation
    {
        ...
        {
            if (model == null) throw new ArgumentException("A model must be specified.");
        ...
        internal Section AddSection(Element element, string title, Format format, string content)
        {
            if (element != null && !Model.Contains(element))
                throw new ArgumentException("The element named " + element.Name +
                                            " does not exist in the model associated with this documentation.");
            ...
            var section = new Section(element, title, CalculateOrder(), format, content);
        ...
        {
            if (string.IsNullOrWhiteSpace(title)) throw new ArgumentException("A title must be specified.");
        }

        private void CheckContentIsSpecified(string title)
        {
            if (string.IsNullOrWhiteSpace(title)) throw new ArgumentException("Content must be specified.");
        }

        private void CheckFormatIsSpecified(Format format)
        {
            if (format == null) throw new ArgumentException("A format must be specified.");
        }
            ...
            if (element == null)
            {
                foreach (var section in Sections)
                    if (section.Element == null && title.Equals(section.Title))
                        throw new ArgumentException("A section with a title of " + title +
            ...
            else
            {
                foreach (var section in Sections)
                    if (element.Id.Equals(section.ElementId) && title.Equals(section.Title))
                        throw new ArgumentException("A section with a title of " + title +
                                                    " already exists for the element named " + element.Name + ".");
            }
        }

        public Decision AddDecision(string id, DateTime date, string title, DecisionStatus status, Format format,
            string content)
        {
            return AddDecision(null, id, date, title, status, format, content);
        }

        public Decision AddDecision(SoftwareSystem softwareSystem, string id, DateTime date, string title,
            DecisionStatus status, Format format, string content)
        {
            ...
            var decision = new Decision(softwareSystem, id, date, title, status, format, content);
        ...
        {
            if (string.IsNullOrWhiteSpace(id)) throw new ArgumentException("An ID must be specified.");
        }

        private void CheckDecisionStatusIsSpecified(DecisionStatus status)
        {
            if (status == null) throw new ArgumentException("A status must be specified.");
        }
            ...
            if (element == null)
            {
                foreach (var decision in Decisions)
                    if (decision.Element == null && id.Equals(decision.Id))
                        throw new ArgumentException("A decision with an ID of " + id +
                                                    " already exists for this workspace.");
            }
            else
            {
