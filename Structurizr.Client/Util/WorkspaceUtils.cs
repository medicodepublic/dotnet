...
namespace Structurizr
{
    public class WorkspaceUtils
    {
        ...
        {
            if (file == null) throw new ArgumentException("The path to a JSON file must be specified.");

            if (!file.Exists) throw new ArgumentException("The specified JSON file does not exist.");
        ...
        {
            if (workspace == null) throw new ArgumentException("A workspace must be provided.");

            if (file == null) throw new ArgumentException("The path to a JSON file must be specified.");

            using (var writer = new StreamWriter(new FileStream(file.FullName, FileMode.Create)))
            {
