...
namespace Structurizr.Examples
{
        ...
        public class BigBankPlc
    {
        ...
        {
            var workspace = new Workspace("Big Bank plc",
            ...
            var customer = model.AddPerson(Location.External, "Personal Banking Customer",
                "A customer of the bank, with personal bank accounts.");

            var internetBankingSystem = model.AddSoftwareSystem(Location.Internal, "Internet Banking System",
                "Allows customers to view information about their bank accounts, and make payments.");
            customer.Uses(internetBankingSystem, "Uses");

            var mainframeBankingSystem = model.AddSoftwareSystem(Location.Internal, "Mainframe Banking System",
                "Stores all of the core banking information about customers, accounts, transactions, etc.");
            mainframeBankingSystem.AddTags(ExistingSystemTag);
            internetBankingSystem.Uses(mainframeBankingSystem, "Uses");

            var emailSystem = model.AddSoftwareSystem(Location.Internal, "E-mail System",
            ...
            var atm = model.AddSoftwareSystem(Location.Internal, "ATM", "Allows customers to withdraw cash.");
            atm.AddTags(ExistingSystemTag);
            atm.Uses(mainframeBankingSystem, "Uses");
            customer.Uses(atm, "Withdraws cash using");

            var customerServiceStaff = model.AddPerson(Location.Internal, "Customer Service Staff",
            ...
            var backOfficeStaff = model.AddPerson(Location.Internal, "Back Office Staff",
            ...
            // containers
            var singlePageApplication = internetBankingSystem.AddContainer("Single-Page Application",
                "Provides all of the Internet banking functionality to customers via their web browser.",
                "JavaScript and Angular");
            singlePageApplication.AddTags(WebBrowserTag);
            var mobileApp = internetBankingSystem.AddContainer("Mobile App",
                "Provides a limited subset of the Internet banking functionality to customers via their mobile device.",
                "Xamarin");
            mobileApp.AddTags(MobileAppTag);
            var webApplication = internetBankingSystem.AddContainer("Web Application",
                "Delivers the static content and the Internet banking single page application.", "Java and Spring MVC");
            var apiApplication = internetBankingSystem.AddContainer("API Application",
                "Provides Internet banking functionality via a JSON/HTTPS API.", "Java and Spring MVC");
            var database = internetBankingSystem.AddContainer("Database",
            ...
            var signinController = apiApplication.AddComponent("Sign In Controller",
                "Allows users to sign in to the Internet Banking System.", "Spring MVC Rest Controller");
            var accountsSummaryController = apiApplication.AddComponent("Accounts Summary Controller",
                "Provides customers with a summary of their bank accounts.", "Spring MVC Rest Controller");
            var resetPasswordController = apiApplication.AddComponent("Reset Password Controller",
                "Allows users to reset their passwords with a single use URL.", "Spring MVC Rest Controller");
            var securityComponent = apiApplication.AddComponent("Security Component",
                "Provides functionality related to signing in, changing passwords, etc.", "Spring Bean");
            var mainframeBankingSystemFacade = apiApplication.AddComponent("Mainframe Banking System Facade",
            ...
            apiApplication.Components.Where(c => "Spring MVC Rest Controller".Equals(c.Technology)).ToList()
            ...
            var developerLaptop = model.AddDeploymentNode("Development", "Developer Laptop", "A developer laptop.",
                "Microsoft Windows 10 or Apple macOS");
            var apacheTomcat = developerLaptop
                .AddDeploymentNode("Docker Container - Web Server", "A Docker container.", "Docker")
                .AddDeploymentNode("Apache Tomcat", "An open source Java EE web server.", "Apache Tomcat 8.x", 1,
            ...
            developerLaptop.AddDeploymentNode("Web Browser", "", "Chrome, Firefox, Safari, or Edge")
                .Add(singlePageApplication);

            var customerMobileDevice =
                model.AddDeploymentNode("Live", "Customer's mobile device", "", "Apple iOS or Android");
            customerMobileDevice.Add(mobileApp);

            var customerComputer =
                model.AddDeploymentNode("Live", "Customer's computer", "", "Microsoft Windows or Apple macOS");
            customerComputer.AddDeploymentNode("Web Browser", "", "Chrome, Firefox, Safari, or Edge")
                .Add(singlePageApplication);

            var bigBankDataCenter = model.AddDeploymentNode("Live", "Big Bank plc", "", "Big Bank plc data center");

            var liveWebServer = bigBankDataCenter.AddDeploymentNode("bigbank-web***",
                "A web server residing in the web server farm, accessed via F5 BIG-IP LTMs.", "Ubuntu 16.04 LTS", 4,
                DictionaryUtils.Create("Location=London and Reading"));
            liveWebServer.AddDeploymentNode("Apache Tomcat", "An open source Java EE web server.", "Apache Tomcat 8.x",
                    1, DictionaryUtils.Create("Xmx=512M", "Xms=1024M", "Java Version=8"))
                .Add(webApplication);

            var liveApiServer = bigBankDataCenter.AddDeploymentNode("bigbank-api***",
                "A web server residing in the web server farm, accessed via F5 BIG-IP LTMs.", "Ubuntu 16.04 LTS", 8,
                DictionaryUtils.Create("Location=London and Reading"));
            liveApiServer.AddDeploymentNode("Apache Tomcat", "An open source Java EE web server.", "Apache Tomcat 8.x",
                    1, DictionaryUtils.Create("Xmx=512M", "Xms=1024M", "Java Version=8"))
                .Add(apiApplication);

            var primaryDatabaseServer = bigBankDataCenter.AddDeploymentNode("bigbank-db01",
                    "The primary database server.", "Ubuntu 16.04 LTS", 1, DictionaryUtils.Create("Location=London"))
                .AddDeploymentNode("Oracle - Primary", "The primary, live database server.", "Oracle 12c");
            primaryDatabaseServer.Add(database);

            var bigBankdb02 = bigBankDataCenter.AddDeploymentNode("bigbank-db02", "The secondary database server.",
                "Ubuntu 16.04 LTS", 1, DictionaryUtils.Create("Location=Reading"));
            bigBankdb02.AddTags(FailoverTag);
            var secondaryDatabaseServer = bigBankdb02.AddDeploymentNode("Oracle - Secondary",
            ...
            var secondaryDatabase = secondaryDatabaseServer.Add(database);

            model.Relationships.Where(r => r.Destination.Equals(secondaryDatabase)).ToList()
                .ForEach(r => r.AddTags(FailoverTag));
            var dataReplicationRelationship =
            ...
            var systemLandscapeView =
                views.CreateSystemLandscapeView("SystemLandscape", "The system landscape diagram for Big Bank plc.");
            systemLandscapeView.AddAllElements();
            systemLandscapeView.PaperSize = PaperSize.A5_Landscape;

            var systemContextView = views.CreateSystemContextView(internetBankingSystem, "SystemContext",
            ...
            var containerView = views.CreateContainerView(internetBankingSystem, "Containers",
            ...
            var componentView = views.CreateComponentView(apiApplication, "Components",
            ...
            var dynamicView = views.CreateDynamicView(apiApplication, "SignIn",
            ...
            var developmentDeploymentView = views.CreateDeploymentView(internetBankingSystem, "DevelopmentDeployment",
            ...
            var liveDeploymentView = views.CreateDeploymentView(internetBankingSystem, "LiveDeployment",
