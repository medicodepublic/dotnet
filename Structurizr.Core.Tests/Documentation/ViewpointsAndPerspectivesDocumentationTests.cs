...
namespace Structurizr.Core.Tests.Documentation
{
    public class ViewpointsAndPerspectivesDocumentationTests : AbstractTestBase
    {
        ...
        private readonly ViewpointsAndPerspectivesDocumentation _template;
        ...
        public void Test_AddAllSectionsWithContentAsStrings()
        {
            ...
            section = _template.AddSystemStakeholdersAndRequirementsSection(_softwareSystem, Format.Markdown,
                "Section 3");
            AssertSection(_softwareSystem, "System Stakeholders and Requirements", Format.Markdown, "Section 3", 3,
                section);
        ...
        {
            Section section;
            var root = new DirectoryInfo("Documentation" + Path.DirectorySeparatorChar + "viewpointsandperspectives");

            section = _template.AddIntroductionSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "01-introduction.md")));
            AssertSection(_softwareSystem, "Introduction", Format.Markdown, "Section 1", 1, section);

            section = _template.AddGlossarySection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "02-glossary.md")));
            AssertSection(_softwareSystem, "Glossary", Format.Markdown, "Section 2", 2, section);

            section = _template.AddSystemStakeholdersAndRequirementsSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "03-system-stakeholders-and-requirements.md")));
            AssertSection(_softwareSystem, "System Stakeholders and Requirements", Format.Markdown, "Section 3", 3,
                section);

            section = _template.AddArchitecturalForcesSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "04-architectural-forces.md")));
            AssertSection(_softwareSystem, "Architectural Forces", Format.Markdown, "Section 4", 4, section);

            section = _template.AddArchitecturalViewsSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "05-architectural-views.md")));
            AssertSection(_softwareSystem, "Architectural Views", Format.Markdown, "Section 5", 5, section);

            section = _template.AddSystemQualitiesSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "06-system-qualities.md")));
            AssertSection(_softwareSystem, "System Qualities", Format.Markdown, "Section 6", 6, section);

            section = _template.AddAppendicesSection(_softwareSystem,
                new FileInfo(Path.Combine(root.FullName, "07-appendices.adoc")));
