    ...
    public class DocumentationTemplateTests : AbstractTestBase
    {
        ...
        public void Test_AddWithContentForSoftwareSystem_ThrowsAnException_WhenThatSectionAlreadyExists()
        {
            ...
            catch (ArgumentException ae)
            {
                ...
                Assert.Equal("A section with a title of Context already exists for the element named Name.",
                    ae.Message);
                Assert.Equal(1, _documentation.Sections.Count);
            }
        }
        ...
        {
            var directory = new DirectoryInfo("Documentation" + Path.DirectorySeparatorChar + "noimages");
        ...
        public void Test_AddImages_ThrowsAnException_WhenTheSpecifiedDirectoryIsNotADirectory()
        {
            try
            {
                var directory = new DirectoryInfo("Documentation" + Path.DirectorySeparatorChar + "example.md");
                _template.AddImages(directory);
                throw new TestFailedException();
            }
            catch (ArgumentException ae)
        ...
        public void Test_AddImages_ThrowsAnException_WhenTheSpecifiedDirectoryDoesNotExist()
        {
            try
            {
                var directory = new DirectoryInfo("Documentation" + Path.DirectorySeparatorChar + "foo");
                _template.AddImages(directory);
                throw new TestFailedException();
            }
            catch (ArgumentException ae)
        ...
        [Fact]
        public void
            Test_AddImages_AddsAllImagesFromTheSpecifiedDirectory_WhenThereAreImageFilesInTheSpecifiedDirectory()
        {
            Assert.Equal(0, _documentation.Images.Count);
            var directory = new DirectoryInfo("Documentation" + Path.DirectorySeparatorChar + "images");
            var images = _template.AddImages(directory);
            Assert.Equal(4, _documentation.Images.Count);
            Assert.Equal(4, images.Count());

            var png = _documentation.Images.Where(i => i.Name.Equals("image.png")).First();
            Assert.Equal("image/png", png.Type);
            Assert.Equal(PngAsBase64, png.Content);
            Assert.True(images.Contains(png));

            var jpg = _documentation.Images.Where(i => i.Name.Equals("image.jpg")).First();
            Assert.Equal("image/jpeg", jpg.Type);
            Assert.Equal(JpegAsBase64, jpg.Content);
            Assert.True(images.Contains(jpg));

            var jpeg = _documentation.Images.Where(i => i.Name.Equals("image.jpeg")).First();
            Assert.Equal("image/jpeg", jpeg.Type);
            Assert.Equal(JpegAsBase64, jpeg.Content);
            Assert.True(images.Contains(jpeg));

            var gif = _documentation.Images.Where(i => i.Name.Equals("image.gif")).First();
        ...
        [Fact]
        public void
            Test_AddImages_AddsAllImagesFromTheSpecifiedDirectory_WhenThereAreImageFilesInTheSpecifiedDirectoryAndSubDirectories()
        {
            Assert.Equal(0, _documentation.Images.Count);
            _template.AddImages(new DirectoryInfo("Documentation"));
            Assert.Equal(8, _documentation.Images.Count);

            var pngInDirectory = _documentation.Images.Where(i => i.Name.Equals("image.png")).First();
            Assert.Equal("image/png", pngInDirectory.Type);
            Assert.Equal(PngAsBase64, pngInDirectory.Content);

            var jpgInDirectory = _documentation.Images.Where(i => i.Name.Equals("image.jpg")).First();
            Assert.Equal("image/jpeg", jpgInDirectory.Type);
            Assert.Equal(JpegAsBase64, jpgInDirectory.Content);

            var jpegInDirectory = _documentation.Images.Where(i => i.Name.Equals("image.jpeg")).First();
            Assert.Equal("image/jpeg", jpegInDirectory.Type);
            Assert.Equal(JpegAsBase64, jpegInDirectory.Content);

            var gifInDirectory = _documentation.Images.Where(i => i.Name.Equals("image.gif")).First();
            Assert.Equal("image/gif", gifInDirectory.Type);
            Assert.Equal(GifAsBase64, gifInDirectory.Content);

            var pngInSubdirectory = _documentation.Images.Where(i => i.Name.Equals("image.png")).First();
            Assert.Equal("image/png", pngInSubdirectory.Type);
            Assert.Equal(PngAsBase64, pngInSubdirectory.Content);

            var jpgInSubdirectory = _documentation.Images.Where(i => i.Name.Equals("image.jpg")).First();
            Assert.Equal("image/jpeg", jpgInSubdirectory.Type);
            Assert.Equal(JpegAsBase64, jpgInSubdirectory.Content);

            var jpegInSubdirectory = _documentation.Images.Where(i => i.Name.Equals("image.jpeg")).First();
            Assert.Equal("image/jpeg", jpegInSubdirectory.Type);
            Assert.Equal(JpegAsBase64, jpegInSubdirectory.Content);

            var gifInSubdirectory = _documentation.Images.Where(i => i.Name.Equals("image.gif")).First();
        ...
        public void Test_AddImage_ThrowsAnException_WhenTheSpecifiedFileDoesNotExist()
        {
            try
            {
                _template.AddImage(new FileInfo("Documentation" + Path.DirectorySeparatorChar +
                                                "some-other-image.png"));
            }
            catch (ArgumentException ae)
            {
                Assert.True(ae.Message.EndsWith("Documentation" + Path.DirectorySeparatorChar +
                                                "some-other-image.png does not exist."));
            }
        }
        ...
        public void Test_AddImage_ThrowsAnException_WhenTheSpecifiedFileIsNotAnImage()
        {
            ...
            {
                Assert.True(ae.Message.EndsWith("Documentation" + Path.DirectorySeparatorChar +
                                                "example.md is not a supported image file."));
            }
        }
        ...
        public void Test_ReadFiles_ThrowsAnException_WhenPassedAnEmptyArray()
        {
            try
            {
                _template.AddContextSection(_softwareSystem);
                throw new TestFailedException();
            }
            catch (ArgumentException ae)
            {
        ...
        {
            var section = _template.AddContextSection(_softwareSystem,
                new FileInfo("Documentation" + Path.DirectorySeparatorChar + "markdown"));
